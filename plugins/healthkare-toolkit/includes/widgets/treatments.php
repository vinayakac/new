<?php
/**
 * Treatments widget class Healthkare
 *
 * @since 2.8.0
 */
class Healthkare_Widget_Treatments extends WP_Widget {

	public function __construct() {

		$widget_ops = array( 'classname' => 'widget_gallery', 'description' => esc_html__( "", "healthkare-toolkit" ) );

		parent::__construct('widget-gallery', esc_html__('Healthkare :: Treatments', "healthkare-toolkit"), $widget_ops);
	...
	public function widget($args, $instance) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Treatments', "healthkare-toolkit" );
				...
				<?php
					while ( $qry->have_posts() ) : $qry->the_post();
						if( has_post_thumbnail() ) {
							?>
								<li><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_post_thumbnail('healthkare_84_84') ?></a></li>
							<?php
						}
					endwhile;	
				?>
	...
	public function form( $instance ) {
		...
		<p>
			...
			<input id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="text" value="<?php echo esc_attr( $number ); ?>" size="9" />
		</p>
		<?php
	}
